...
const FailedTask = ({userno,data,setloggedIn}) => {
  ...
  return (
    <div className='relative min-h-full w-100 bg-rose-600 rounded-3xl shrink-0 p-6 flex flex-col gap-8 text-white'>
            <div className='flex justify-between items-center'>
                <span className='text-2xl bg-orange-600 px-2 py-1 rounded-xl'>Failed</span>
            ...
            <div className='absolute bottom-5 text-xl left-0 w-full box-border flex items-center justify-center h-5'>
                <button className='bg-blue-600 transition-all px-4 py-1 rounded-full hover:scale-115 hover:bg-blue-700 hover:text-amber-50' onClick={deleteHandler}>Delete</button>
            </div>
        </div>
  )
}
